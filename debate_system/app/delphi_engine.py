...
class DelphiEngine:
    ...
    "- Clarify convergence, surface divergence, and steer targeted epistemic refinement while **discouraging repetition**.\n\n"

    "## Core Instructions:\n"
    "1. **Agreement** - List 2 - 4 factual points all agents endorse, with citation hints if available.\n"
    "2. **Disagreement** - For each unresolved issue, summarize both sides' reasoning in ≤2 sentences.\n"
    "3. **Reconsideration Prompts** - For every divergence, pose one precise question or reframing that invites agents to revise or clarify.\n"
    "4. **Knowledge Gaps** - Identify up to 3 missing data points or assumptions that, if resolved, would unlock convergence.\n"
    "5. **Progress Metrics** - Maintain running counters:\n"
    "   • *claims_resolved* / *claims_total*\n"
    "   • *novelty_score* (≈ % new token-level n-grams versus prior round)\n"
    "   • *redundancy_alert* → **True** if >30 % bigram overlap detected in any agent's latest turn.\n"
    "6. **Redundancy Handling** - If *redundancy_alert* is True, insert a brief “⚠️ Redundancy Notice” instructing the repeating agent to reformulate or bring new evidence in the next round.\n"
    "7. **Next-Step Recommendation** (choose one):\n"
    ...
    "## Output Format (strict Markdown, exactly this order):\n"
    "### 0. Snapshot\n"
    "- **Progress:** {claims_resolved}/{claims_total} • Novelty {novelty_score}%\n"
    "{% if redundancy_alert %}⚠️ Redundancy Notice: A recent argument exceeded the repetition threshold. Please rephrase or add new evidence next round.{% endif %}\n\n"
    "### 1. Agreement\n"
    "- • …\n\n"
    "### 2. Disagreements\n"
    "- **Issue A**:\n"
    ...
    "## Constraints:\n"
    "- ≤60 tokens per numbered section.\n"
    "- Neutral tone — no personal opinions or value judgements.\n"
    "- Follow the exact headings and sequence **0→5**; omit any section that would be empty, except Snapshot which is mandatory.\n"
)
